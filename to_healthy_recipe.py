...
'sugar': 'unsweetened applesauce', 'peanut butter': 'natural peanut butter',
...
def to_healthy_recipe(recipe):
	...
	for ingredient in recipe['ingredients']:
		if checkSodiumLevel(Sodium_nutri) is True:
			if 'salt' in ingredient and any(x in salt_sodium.keys() for x in ingredient):
				...
				reduced_quantity = str(int(recipe['ingredients'][index][0])/2)
	...
	return recipe
			...
			# 	sodium = int(ingredient[0])*butter_sodium[ingredient[1]]
			# 	reduced_per_serving = sodium / servings
			# 	Sodium_nutri = Sodium_nutri - reduced_per_serving 
